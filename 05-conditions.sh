...
# [ -r new.txt ] -> True if new.txt is readable.
# [ -w new.txt ] -> True if new.txt is writable.
# [ -x new.txt ] -> True if new.txt is executable.
...
if [ "$currency" == usd ]; then
  ...
  echo enter proper currency format usd or aud.
fi

## In Expressions always use double quotes for variables & strings. For numbers it is not necessary.
